...
from authentik.policies.hooks import (
    register_policy_request_processor,
    unregister_policy_request_processor,
)
...
class TestListPolicyEngine(TestCase):
    ...
    def test_list_engine_runs_policy_request_processors_after_obj_is_set(self):
        """Processors must see each object's PolicyRequest, matching PolicyEngine."""
        seen_pks: list = []

        def processor(policy_request):
            if policy_request.obj is not None:
                seen_pks.append(policy_request.obj.pk)
                policy_request.context["list_engine_marker"] = True

        register_policy_request_processor(processor)
        self.addCleanup(unregister_policy_request_processor, processor)

        policy = ExpressionPolicy.objects.create(
            name=generate_id(),
            expression="return request.context.get('list_engine_marker', False)",
        )
        PolicyBinding.objects.create(target=self.obj_a, policy=policy, order=0)
        PolicyBinding.objects.create(target=self.obj_b, policy=policy, order=0)
        engine = ListPolicyEngine(self.objs, self.user)
        engine.use_cache = False

        passing = set(engine.build().result.values_list("pk", flat=True))

        self.assertEqual(set(seen_pks), {self.obj_a.pk, self.obj_b.pk})
        self.assertEqual(passing, {self.obj_a.pk, self.obj_b.pk, self.obj_c.pk})
